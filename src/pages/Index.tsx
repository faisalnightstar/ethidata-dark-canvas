...
export default function Index() {
  return (
    <Layout showBackground3D>
      <section className="relative min-h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          <HeroStagger className="mx-auto max-w-4xl text-center">
            <HeroItem>
              <span className="mb-6 inline-block rounded-full bg-primary/10 px-4 py-2 text-body-sm font-medium text-primary border border-primary/20">
            ...
            <HeroItem>
              <p className="mx-auto mt-6 max-w-2xl text-body-lg text-muted-foreground">
                We partner with enterprises to design, build, and scale transformative technology solutions that drive innovation and growth.
              </p>
            </HeroItem>
            <HeroItem>
              <div className="mt-10 flex flex-col justify-center gap-4 sm:flex-row">
                <GlowButton href="/services" size="lg" icon className="magnetic">
                  Explore Services
                </GlowButton>
                <GlowButton href="/contact" variant="outline" size="lg" className="magnetic">
                  Get in Touch
                </GlowButton>
              </div>
            </HeroItem>
          ...
          <div className="relative bottom-8 left-1/2 -translate-x-1/2 mt-32 flex justify-center items-center">
            <div className="absolute text-muted-foreground scroll-indicator">
      ...
      <section className="section-padding bg-surface-1 tech-pattern">
        <div className="container-custom">
          ...
          <StaggerGrid className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((service, index) => (
              <StaggerItem key={index}>
                <Link to="/services" className="magnetic">
                  <MotionCard>
                    <ServiceCard {...service} />
                  </MotionCard>
                </Link>
          ...
          <MotionSection className="mt-12 text-center">
            <GlowButton href="/services" variant="outline" icon className="magnetic">
              View All Services
            </GlowButton>
          </MotionSection>
        </div>
          ...
          <StaggerGrid className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat, index) => (
              <StaggerItem key={index} className="text-center">
                <AnimatedCounter value={stat.value} className="text-display-lg gradient-text" />
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            <MotionSection>
              ...
              <div className="space-y-4 text-body-md text-muted-foreground">
                <p>EthiData & Technologies (formerly known as PairUp Labs) was founded with a simple mission: to help enterprises harness the power of technology to achieve their goals.</p>
                <p>Our team of world-class engineers, architects, and strategists work together to deliver solutions that make a real impact.</p>
              </div>
              <div className="mt-8">
                <GlowButton href="/about" icon className="magnetic">
                  Learn More About Us
                </GlowButton>
              </div>
            </MotionSection>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    {stats.slice(0, 4).map((stat, index) => (
                      <div key={index} className="text-center">
                        <div className="text-h2 gradient-text">{stat.value}</div>
                        <div className="mt-1 text-body-xs text-muted-foreground">{stat.label}</div>
                      </div>
                    ))}
                  </div>
                </GlassCard>
                <div className="absolute -z-10 -inset-4 rounded-2xl bg-gradient-to-br from-primary/10 to-secondary/10 blur-2xl" />
              </div>
            </MotionSection>
          </div>
        </div>
      ...
      <section className="py-16 border-y border-border/50 bg-surface-1">
        <div className="container-custom">
          <MotionSection>
            <p className="mb-8 text-center text-body-sm font-medium uppercase tracking-widest text-muted-foreground">
          ...
          <StaggerGrid className="flex flex-wrap items-center justify-center gap-8 md:gap-16">
            {partners.map((partner, index) => (
              <StaggerItem key={index}>
                <div className="text-h4 font-semibold text-muted-foreground/50 transition-colors hover:text-primary magnetic">
                  {partner}
                </div>
              </StaggerItem>
            ))}
        ...
        <div className="container-custom">
          ...
          <StaggerGrid className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {caseStudies.map((study, index) => (
              <StaggerItem key={index}>
                <Link to="/case-studies" className="magnetic">
                  <MotionCard>
                    <CaseStudyCard {...study} />
                  </MotionCard>
                </Link>
          ...
          <MotionSection className="mt-12 text-center">
            <GlowButton href="/case-studies" variant="outline" icon className="magnetic">
